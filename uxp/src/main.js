...
async function populateProjectInfo() {
  ...
  if (!project) {
    ...
    console.log(`Active PP project: ${project.name}`);
    log(`Active PP project: ${project.name}`);
    ...
    if (!sequence) {
      ...
      sequence.getAudioTrackCount().then((audioTrackCount) => {
        log(`Audio track count: ${audioTrackCount}`);
      });
